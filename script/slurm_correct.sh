...
if [ "${TRAINING_MODE}" == "Test" ]; then
    ...
    python -u correct.py --launcher slurm --tcp_port $PORT ${PY_ARGS}
    # --model_name ${MODEL_NAME} --width_threshold 0
else
    echo "using srun for training"
    srun -p ${TRAINING_MODE} --job-name=${JOB_NAME} -n ${GPUS} --gres=gpu:${GPUS} --ntasks-per-node=${GPUS} --cpus-per-task=${CPUS_PER_TASK} \
    python -u correct.py --launcher slurm --tcp_port $PORT ${PY_ARGS}
fi
